...
-- {{{ Personal keybindings
  ...
  -- ctrl + shift + ...
  awful.key({ modkey1, "Shift" }, "Escape", function() awful.util.spawn(string.format("%s -e btop", terminal)) end),
  ...
  awful.key({ altkey, "Shift" }, "j", function() awful.tag.incnmaster(1, nil, true) end,
    { description = "increase the number of master clients", group = "layout" }),
  awful.key({ altkey, "Shift" }, "k", function() awful.tag.incnmaster(-1, nil, true) end,
...
clientkeys = my_table.join(
  ...
  awful.key({ modkey, "Shift" }, "l", function(c) c:move_to_screen() end,
    { description = "move to screen", group = "client" })
)
...
awful.spawn.with_shell("picom -b --config  $HOME/.config/qtile/picom.conf")

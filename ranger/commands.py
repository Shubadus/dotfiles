...
class unzip(Command):
    """Easily unzip files compressed with the ZIP method"""
    def execute(self):
        if self.arg(1):
            target_filename = self.rest(1)
        else:
        ...
        if not os.path.exists(target_filename):
            self.fm.notify("The specifed file cannot be found", bad=True)
            return

        self.fm.execute_command(f"unzip {target_filename}")
